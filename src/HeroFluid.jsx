...
void main() {
  ...
  // Accumulate disturbance from clicks — warp the sampling point
  ...
  for (int i = 0; i < 3; i++) {
    ...
    // Fade: fast surge then slow settle
    float surge  = exp(-age * 3.0);          // sharp push at impact
    float settle = exp(-age * 0.8) * 0.3;    // slow return to normal
    ...
    // Radial push outward from click point, decays with distance
    float influence = exp(-d * 8.0);
    vec2 pushDir = d > 0.001 ? normalize(diff) : vec2(0.0);

    // Swirl: tangential component that rotates around the click
  ...
  // Subtract disturbance so visual content moves away from click
  vec2 pd = p - disturbance;

  // Normal water layers — sampled from disturbed position
  vec2 q = vec2(
...
export default function HeroFluid() {
  ...
  useEffect(() => {
    ...
    const onClick = (e) => {
      // Only fire if click is within hero (first 100vh)
