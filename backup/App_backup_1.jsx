  ...
  return (
    <nav style={{ position:'fixed', top:0, left:0, right:0, zIndex:200, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 clamp(20px,4vw,60px)', height:'58px', background: scrolled?'rgba(12,13,15,0.92)':'transparent', backdropFilter: scrolled?'blur(16px)':'none', borderBottom: scrolled?'1px solid var(--border)':'1px solid transparent', transition:'background 0.3s, border-color 0.3s' }}>
      ...
      <div style={{ display:'flex', gap:'28px', alignItems:'center' }}>
        ...
        <a href="https://calendly.com/michael-devhousetech/30min" target="_blank" rel="noreferrer" className="px-btn px-btn-dark" style={{fontSize:'11px'}}>Book a call</a>
      </div>
    </nav>
  )
}
    ...
    <section style={{ position:'relative', height:'100vh', display:'flex', flexDirection:'column', overflowX:'clip', overflowY:'hidden' }}>
      ...
      <div style={{ position:'relative', zIndex:4, height: isMobile?'40%':'32%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'0 clamp(24px,5vw,80px)' }}>
        ...
        <div style={{ display:'flex', gap:'12px', justifyContent:'center' }}>
          <a href="#work" className="px-btn px-btn-dark">View work</a>
          <a href="#contact" className="px-btn px-btn-white">About</a>
        </div>
      </div>
...
function SkillsTable() {
  ...
  useEffect(() => {
    ...
    // Register trigger for Hero button
    window.__triggerSkills = () => setPhase('in')
              ...
              {SKILLS.map((s, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    <div style={{ fontSize:'clamp(22px,2.6vw,42px)', fontWeight:700, letterSpacing:'0.03em', color:'var(--offwhite)', fontFamily:"'Biennale', sans-serif", lineHeight:1 }}>{s.skill}</div>
...
function About({ onEnter, onExitTop }) {
  ...
  // Separate handler for game slide — scroll up exits game, no scroll-down trap
  useEffect(() => {
    if (phase !== 'active') return
    let cooldown = false
    const onGameWheel = (e) => {
      if (activeIdxRef.current !== 3) return
      if (e.deltaY >= 0) return // let scroll down pass through to game
      e.preventDefault()
      if (cooldown) return
      cooldown = true
      setTimeout(() => { cooldown = false }, 600)
      setActiveIdx(2) // back to product slide
    }
  ...
  const interests = [
    ['Currently into','Building SaaS products, AI tooling'],
    ['Outside work','Lorem ipsum dolor sit amet consectetur'],
    ['Hobbies','Lorem ipsum, dolor sit, amet'],
    ['Watching','Lorem ipsum series, documentaries'],
    ['Reading','Lorem ipsum on design, business, tech'],
    ['Music','Lorem ipsum genres and artists'],
  ]
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,420px),1fr))', gap: 'clamp(40px,6vw,100px)' }}>
            <div>
              ...
              <p style={{ fontSize: '15px', color: 'var(--silver)', lineHeight: 1.85 }}>
                I have a strong eye for luxury design and genuinely enjoy working with premium brands, high-end real estate clients, and anyone who takes their digital presence seriously. Currently open to <strong style={{ color: 'var(--offwhite)', fontWeight: 500 }}>freelance, full-time, and white-label partnerships</strong>.
              </p>
            </div>
            <div>
              {facts.map(([label, val]) => (
